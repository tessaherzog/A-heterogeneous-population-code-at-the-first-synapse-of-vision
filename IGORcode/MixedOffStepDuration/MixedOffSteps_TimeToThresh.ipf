...
Function TimeToThreshold_MOS(trace_decon, SnipMat, Snips_MaxAmp, Snips_MaxAmpXlocs)
wave trace_decon, SnipMat, Snips_MaxAmp, Snips_MaxAmpXlocs

// TimeToThreshold_MOS(trace_decon, RespSnips_Ex, RespPeakAmps_Ex_Thresh, RespPeakxLocs_Ex_Thresh)
...
duplicate/o/r=[116000+100,118000] trace_decon, baseline_end
...
variable nStimConds = dimsize(SnipMat,2)

make/o/n=(nStimReps, nStimConds) ThreshPeak_Xlocs // make array to save xLoc of half peak.
for (i=0;i<nStimConds;i+=1) // loop over layers (stim conditions)
	for (j=0;j<nStimReps;j+=1) // loop over rows (Stim reps)
		...
		elseif (Snips_MaxAmp[j][i] < threshold)
			threshpeaktime1 = NaN // include so that is response never goes below half peak, value remains as NaN
	     	threshpeakvalue1 = NaN
    	 	threshpeaktime2 = NaN
    	 	threshpeakvalue2 = NaN
...
//			Redimension/N=-1 tempSnip
	...
	//	print "so, real xloc value where response crosses threshold is " + num2str(realxvalue)
		ThreshPeak_Xlocs[j][i] = realxvalue
	endfor
endfor
killwaves tempsnip
...
make/o/n=(nStimConds) TimeToThresh_Mean
make/o/n=(nStimConds) TimeToThresh_SD
make/o/n=(nStimConds) TimeToThresh_Var

for (i=0;i<nStimConds;i+=1)
	duplicate/o/r=[][i] TimeToMSDThresh, TimeToMSDThresh_pcond
	wavestats/q TimeToMSDThresh_pcond
	TimeToThresh_Mean[i] = v_avg
	TimeToThresh_SD[i] = v_sdev
	TimeToThresh_Var[i] = v_sdev^2
endfor
killwaves TimeToMSDThresh_pcond
...
for (j=0;j<nStimConds;j+=1)
	display/k=1 snipMat[][0][j]
	for (i=1;i<nStimReps;i+=1)
		appendtograph SnipMat[][i][j]
	endfor
	...
	Label bottom "Time (s)"
	variable meanTTthresh = TimeToThresh_Mean[j]
	variable SDTTthresh = TimeToThresh_SD[j]
